...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($client_name) || empty($client_phone) || empty($car_license_no) || empty($car_engine_no) || empty($appointment_date) || empty($mechanic_id)) {
        ...
        // Validation 1: Check if the client already has an appointment on this specific date
        // Assuming client_phone or car_license_no identifies the client uniquely
        ...
        if ($client_result->num_rows > 0) {
            ...
            // Validation 2: Check if the mechanic is fully occupied on this date (Max 4)
            $MAX_APPOINTMENTS = 4;
            ...
            if ($row['total'] >= $MAX_APPOINTMENTS) {
                // The mechanic is fully occupied. We notify the client instead of auto-assigning.
                $message = "Sorry, the selected mechanic is fully booked on this date. Please choose another mechanic or date.";
                $messageType = "error";
            } else {
                // Both checks passed, insert record
    ...
    // Set session variable to display message on redirect
